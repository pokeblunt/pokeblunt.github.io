    ...
    for (var player of players) {
        ...
        for (var creature of player_creatures) {
            table_rows.push(create_element({tag: "tr", children: [
                ...
                create_element({tag: "td", innerHTML: creature.name, classList: ["stat-link"], attributes: {"data-dex": creature.id}}),
    ...
    draw(creatures) {
        var dom_stats_table = create_element({tag: "table", classList: ["players", "standard"], children: [
            ...
            create_element({tag: "tr", classList: [], children: [
                create_element({tag: "td", innerHTML: this.name, classList: ["winner", this.css_class, "stat-link"], attributes: {"data-player": this.id}}),
        ...
        return create_element({tag: "div", classList: ["event", "grey", "trainer", this.css_class], children: [
            create_element({tag: "div", classList: ["header", "stat-link"], innerHTML: "Trainer: " + this.name, attributes: {"data-player": this.id}}),
            create_element({tag: "div", classList: ["body"], children: [dom_stats_table].concat(dom_creature_arts)}),
        ]});
    }
    draw_sprite() {
        return create_element({tag: "img", attributes: {"src": this.trainer_sprite, "data-player": this.id, title: "Battle stats for " + this.name}, classList: ["trainer_sprite", "stat-link"]});
    }
    draw_large_sprite() {
        return create_element({tag: "img", attributes: {"src": this.trainer_sprite, "data-player": this.id, title: "Battle stats for " + this.name}, classList: ["trainer_sprite", "large", "stat-link"]});
    }
}
...
class Creature {
    ...
    draw_sprite() {
        return create_element({tag: "img", classList: ["sprite", "stat-link"], attributes: {
            "data-dex": this.id,
            title: "Battle stats for " + this.name,
    ...
    draw_art_with_discription() {
        // Stays an <a> so the existing `a.art-holder` layout rules still apply.
        // The stat panel intercepts the click; the href is kept as a middle-click fallback.
        var href = "https://bulbapedia.bulbagarden.net/wiki/" + this.name.replace(" ", "_");
        return create_element({tag: "a", attributes: {href: href, target: "_blank", "data-dex": this.id},
                               classList: ["art-holder", "stat-link"], children: [
            this.draw_art(),
