...
function draw_creature_statistics_table(players, creatures, dom) {
    ...
    var outer_dom = document.querySelector("#put-pokemon-stats-table-here div");
...
class Event {
    ...
    draw(children, title, css_class) {
        return create_element({tag: "div", classList: ["event", css_class, "compact"], attributes: {onclick: "toggle_expanded(this)"}, children: [
...
class Champion extends Event {
    ...
    draw(players, creatures) {
        var dom_date = create_element({tag: "div", innerHTML: this.date.toLocaleDateString(), style: "font-size: 11px; color: #666; margin-bottom: 5px;"});
        var dom_description = create_element({tag: "div", innerHTML: this.description, style: "font-size: 12px; margin-bottom: 8px; font-style: italic;"});

        let player_ids = this.teams.map(({ player_id }) => player_id);
        var players_ordered = Player.get_by_ids(players, player_ids);  // Order players by rank (in order of teams)

        // Create compact champion display
        var champion_player = players_ordered[0];
        var champion_team = this.teams[0];
        var dom_champ_match = create_element({tag: "div", classList: ["match-row", "champion"], style: "background: linear-gradient(135deg, #f39c12, #e67e22); color: white; font-weight: bold; padding: 8px;", children: [
            create_element({tag: "div", innerHTML: "🏆", style: "font-size: 14px; min-width: 25px;"}),
            create_element({tag: "div", classList: ["match-player", "winner"], children: [
                create_element({tag: "img", attributes: {src: champion_player.trainer_sprite}, style: "width: 20px; height: 20px;"}),
                create_element({tag: "span", classList: ["player-name"], innerHTML: champion_player.name, style: "color: white;"}),
                create_element({tag: "div", classList: ["match-pokemon"], children: champion_team.creature_ids.slice(0, 6).map(creature_id => {
                    var creature = creatures[creature_id - 1];
                    return creature.draw_sprite();
                })})
            ]}),
            create_element({tag: "span", innerHTML: "CHAMPION", style: "font-size: 10px; color: #f1c40f; margin: 0 5px; font-weight: bold;"}),
            create_element({tag: "div", style: "flex: 1;"})
        ]});

        return super.draw([dom_date, dom_description, dom_champ_match], "🏆 Champion", "grey");
    }
}
...
class Tournament extends Event {
    ...
    draw_with_is_official(players, creatures, is_official) {
        ...
        // Create compact matches display
        var dom_matches_container = create_element({tag: "div", classList: ["matches-container"]});
        
        for (let i = 0; i < this.matches.length; i++) {
            let match = this.matches[i];
            var winner_player = players[match.win_team.player_id];
            var loser_player = players[match.lose_team.player_id];
            
            // Create winner player element
            var winner_pokemon_sprites = match.win_team.creature_ids.slice(0, 6).map(creature_id => {
                var creature = creatures[creature_id - 1];
                return creature.draw_sprite();
            });
            var winner_element = create_element({tag: "div", classList: ["match-player", "winner"], children: [
                create_element({tag: "img", attributes: {src: winner_player.trainer_sprite}, style: "width: 18px; height: 18px;"}),
                create_element({tag: "span", classList: ["player-name"], innerHTML: winner_player.name}),
                create_element({tag: "div", classList: ["match-pokemon"], children: winner_pokemon_sprites})
            ]});
            
            // Create loser player element
            var loser_pokemon_sprites = match.lose_team.creature_ids.slice(0, 6).map(creature_id => {
                var creature = creatures[creature_id - 1];
                return creature.draw_sprite();
            });
            var loser_element = create_element({tag: "div", classList: ["match-player"], children: [
                create_element({tag: "img", attributes: {src: loser_player.trainer_sprite}, style: "width: 18px; height: 18px;"}),
                create_element({tag: "span", classList: ["player-name"], innerHTML: loser_player.name}),
                create_element({tag: "div", classList: ["match-pokemon"], children: loser_pokemon_sprites})
            ]});
            
            // Create match row
            var match_row = create_element({tag: "div", classList: ["match-row"], children: [
                create_element({tag: "div", innerHTML: "M" + (i + 1), style: "font-size: 10px; font-weight: bold; color: #666; min-width: 25px;"}),
                winner_element,
                create_element({tag: "span", innerHTML: "vs", style: "font-size: 10px; color: #999; margin: 0 5px;"}),
                loser_element
            ]});
            
            dom_matches_container.appendChild(match_row);
        }

        if (is_official) {
            return super.draw([dom_date, dom_description, dom_matches_container], "T" + this.number, "tournament");
        }
        return super.draw([dom_date, dom_description, dom_matches_container], "UT" + this.number, "unofficial-tournament");
    }
    draw(players, creatures) {
...
class Draft extends Event {
    ...
    draw(players, creatures) {
        ...
        // Create compact draft display
        var dom_draft_container = create_element({tag: "div", classList: ["matches-container"]});
        ...
        for (var i = 0; i < this.creatures_drafted.length; i++) {
            ...
            var draft_row = create_element({tag: "div", classList: ["match-row"], children: [
                create_element({tag: "div", innerHTML: "#" + (i + 1), style: "font-size: 10px; font-weight: bold; color: #666; min-width: 25px;"}),
                create_element({tag: "div", classList: ["match-player"], children: [
                    create_element({tag: "img", attributes: {src: player.trainer_sprite}, style: "width: 18px; height: 18px;"}),
                    create_element({tag: "span", classList: ["player-name"], innerHTML: player.name}),
                    create_element({tag: "div", children: [creature.draw_sprite()]}),
                    create_element({tag: "span", style: "font-size: 9px; color: #666; margin-left: 5px;", innerHTML: creature.name})
                ]})
            ]});
            
            dom_draft_container.appendChild(draft_row);
        }

        return super.draw([dom_date, dom_description, dom_draft_container], "D" + this.number, "draft");
    }
}
...
class Trade extends Event {
    ...
    draw(players, creatures) {
        ...
        // Create compact trade display
        var dom_trade_container = create_element({tag: "div", classList: ["matches-container"]});
        ...
        for (var creature_team_change of creature_team_changes) {
            ...
            var trade_row = create_element({tag: "div", classList: ["match-row"], children: [
                create_element({tag: "div", innerHTML: "#" + creature.id, style: "font-size: 10px; font-weight: bold; color: #666; min-width: 25px;"}),
                create_element({tag: "div", classList: ["match-player"], children: [
                    create_element({tag: "div", children: [creature.draw_sprite()]}),
                    create_element({tag: "span", style: "font-size: 9px; color: #666; margin-left: 5px;", innerHTML: creature.name})
                ]}),
                create_element({tag: "span", innerHTML: "→", style: "font-size: 12px; color: #27ae60; margin: 0 5px; font-weight: bold;"}),
                create_element({tag: "div", classList: ["match-player"], children: [
                    create_element({tag: "span", classList: ["player-name"], innerHTML: to_player_name, style: "color: #27ae60; font-weight: bold;"})
                ]})
            ]});
            
            dom_trade_container.appendChild(trade_row);
        }
        
        return super.draw([dom_date, dom_description, dom_trade_container], "T" + this.number, "trade");
    }
    update_statistics(players, creatures) {
        var creature_ids = [];
...
class Round extends Event {
    ...
    draw(players, creatures) {
        ...
        return super.draw([dom_date, dom_description, dom_matches_container], "R" + this.number, "round");
    }
    update_statistics(players, creatures) {
        var player_ids, n_wins, n_loss;
        [player_ids, n_wins, n_loss] = Match.get_player_ids_wins_and_loses(this.matches);
