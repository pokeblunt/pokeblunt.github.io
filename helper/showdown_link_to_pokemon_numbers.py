...
def main() -> None:
    match = Match("https://replay.pokemonshowdown.com/gen9doublescustomgame-2263042311.json")
    print(match.to_pokeblunt())
...
{{
    ...
    def _p1_pkmn_names(self) -> list[str]:
        ...
        return [s.split("|poke|p1|", 1)[1].split(",", 1)[0] for s in matches]
    ...
    def _p2_pkmn_names(self) -> list[str]:
        ...
        assert len(matches) == 6, f"Found {len(matches)} matches for pokemon names in logs"
        return [s.split("|poke|p2|", 1)[1].split(",", 1)[0] for s in matches]
